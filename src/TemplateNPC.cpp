...
void sTemplateNPC::ExtractGearTemplateToDB(Player* player, std::string& playerSpecStr, ChatHandler* handler)
{
    for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        if (equippedItem)
        {
            handler->SendGlobalGMSysMessage("Adding to slot: %u, item: %u");
            sLog->outString("Adding to slot: %u, item: %u", i,  equippedItem->GetEntry());
            if (player->getRace() == RACE_HUMAN)
            {
                    sLog->outString("INSERT INTO template_npc_human (`playerClass`, `playerSpec`, `pos`, `itemEntry`, `enchant`, `socket1`, `socket2`, `socket3`, `bonusEnchant`, `prismaticEnchant`) VALUES ('%s', '%s', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u');"
            ...
            {
                sLog->outString("INSERT INTO template_npc_alliance (`playerClass`, `playerSpec`, `pos`, `itemEntry`, `enchant`, `socket1`, `socket2`, `socket3`, `bonusEnchant`, `prismaticEnchant`) VALUES ('%s', '%s', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u');"
                    , GetClassString(player).c_str(), playerSpecStr.c_str(), equippedItem->GetSlot(), equippedItem->GetEntry(), equippedItem->GetEnchantmentId(PERM_ENCHANTMENT_SLOT),
                    equippedItem->GetEnchantmentId(SOCK_ENCHANTMENT_SLOT), equippedItem->GetEnchantmentId(SOCK_ENCHANTMENT_SLOT_2), equippedItem->GetEnchantmentId(SOCK_ENCHANTMENT_SLOT_3),
                    equippedItem->GetEnchantmentId(BONUS_ENCHANTMENT_SLOT), equippedItem->GetEnchantmentId(PRISMATIC_ENCHANTMENT_SLOT));
            ...
            {
                 sLog->outString("INSERT INTO template_npc_horde (`playerClass`, `playerSpec`, `pos`, `itemEntry`, `enchant`, `socket1`, `socket2`, `socket3`, `bonusEnchant`, `prismaticEnchant`) VALUES ('%s', '%s', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u');"
                    , GetClassString(player).c_str(), playerSpecStr.c_str(), equippedItem->GetSlot(), equippedItem->GetEntry(), equippedItem->GetEnchantmentId(PERM_ENCHANTMENT_SLOT),
                    equippedItem->GetEnchantmentId(SOCK_ENCHANTMENT_SLOT), equippedItem->GetEnchantmentId(SOCK_ENCHANTMENT_SLOT_2), equippedItem->GetEnchantmentId(SOCK_ENCHANTMENT_SLOT_3),
                    equippedItem->GetEnchantmentId(BONUS_ENCHANTMENT_SLOT), equippedItem->GetEnchantmentId(PRISMATIC_ENCHANTMENT_SLOT));
...
void sTemplateNPC::ExtractTalentTemplateToDB(Player* player, std::string& playerSpecStr)
{
    QueryResult result = CharacterDatabase.PQuery("SELECT spell FROM character_talent WHERE guid = '%u' "
        "AND specMask = '%u';", player->GetGUID(), player->GetActiveSpec());

    if (!result)
    {
        player->GetSession()->SendAreaTriggerMessage("There was no talent spec found.");
        return;
    }
...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        static std::vector<ChatCommand> TemplateNPCTable =
        {
            ...
            { "save",      SEC_ADMINISTRATOR, true , &HandleSaveTemplateNPCCommand, "" },
            { "load",      SEC_ADMINISTRATOR, true , &HandleLoadTemplateNPCCommand, "" },
        };
        ...
        static void EquipFullTemplateGear(Player* player, std::string& playerSpecStr) // Merge
        {
            ...
            sTemplateNpcMgr->sTalentsSpec = playerSpecStr;
            sTemplateNpcMgr->LearnTemplateTalents(player);
            sTemplateNpcMgr->LearnTemplateGlyphs(player);
            sTemplateNpcMgr->EquipTemplateGear(player);
            sTemplateNpcMgr->LearnPlateMailSpells(player);
    ...
    static bool HandleSaveTemplateNPCCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
        ...
        handler->SendGlobalGMSysMessage("Template Save Action.");
        std::string name = args;
        sTemplateNpcMgr->ExtractGearTemplateToDB(player, name, handler);
        sTemplateNpcMgr->ExtractTalentTemplateToDB(player, name);
        // sTemplateNpcMgr->ExtractGlyphsTemplateToDB(player, name);
        handler->SendGlobalGMSysMessage("Template Complete.");


        return true;
    }

    static bool HandleLoadTemplateNPCCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
        ...
        std::string name = args;
        EquipFullTemplateGear(player, name);
        handler->SendGlobalGMSysMessage("Template Load Action");
